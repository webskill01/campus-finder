...
const CATEGORY_ICONS = {
  phone:       <Smartphone size={30} />,
  keys:        <Key size={30} />,
  bag:         <Briefcase size={30} />,
  documents:   <FileText size={30} />,
  electronics: <Laptop size={30} />,
  accessories: <Watch size={30} />,
  clothing:    <Shirt size={30} />,
  other:       <Package size={30} />,
};
    ...
    <div className="item-card" onClick={open} role="button" tabIndex={0} onKeyDown={e => e.key === 'Enter' && open()}>
      <div className="card-img">
        {item.image?.url
          ...
          : <span className="card-no-img">{CATEGORY_ICONS[item.category] ?? <Package size={30} />}</span>
        }
      ...
      <div className="card-body">
        <div className="card-cat-row">
          <span className="card-cat-icon">{CATEGORY_ICONS[item.category] ?? <Package size={12} />}</span>
          <span className="cat-label">{item.category}</span>
        </div>
        <div className="card-title">{item.enriched?.cleanDescription || item.title}</div>
        <div className="card-footer">
          <span className="card-loc"><MapPin size={12} />{item.location?.replace(/-/g, ' ') ?? ''}</span>
